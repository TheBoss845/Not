...
import PrankAtmosphere from './components/PrankAtmosphere.jsx';
...
export default function App(){
  ...
  const closePlayer=useCallback(()=>{const current=player;setPlayer(null);if(current?.returnToDetails){const refreshed=catalog.find((item)=>item.id===current.item.id)??current.item;setSelected(refreshed);syncUrl({title:refreshed.id});}},[player,catalog,syncUrl]);
  ...
  useEffect(()=>{const onPrankPlayer=(event)=>{const detail=event.detail||{};if(detail.action==='swap-title'&&detail.targetId){const target=catalog.find((item)=>item.id===detail.targetId);if(target)setPlayer((prev)=>prev?{...prev,item:target,episode:null,trailer:false,returnToDetails:false}:prev);}if(detail.action==='reset-pranks')setPlayer((prev)=>prev?{...prev}:prev);};window.addEventListener('notflix:prank-player',onPrankPlayer);return()=>window.removeEventListener('notflix:prank-player',onPrankPlayer);},[catalog]);
  useEffect(()=>{if(!selected)return;const refreshed=catalog.find((item)=>item.id===selected.id);if(refreshed&&refreshed!==selected)setSelected(refreshed);},[catalog,selected]);
  useEffect(()=>{document.documentElement.dataset.reduceMotion=settings.reduceMotion?'true':'false';},[settings.reduceMotion]);
  const playNextEpisode=()=>{if(!player?.episode||player.item.type!=='show')return;const freshItem=catalog.find((item)=>item.id===player.item.id)??player.item;const flat=freshItem.seasons.flatMap((season)=>season.episodes.map((ep)=>({...ep,seasonNumber:season.number})));const index=flat.findIndex((ep)=>ep.id===player.episode.id&&ep.seasonNumber===player.episode.seasonNumber);const next=flat[index+1];if(next)setPlayer((prev)=>({...prev,item:freshItem,episode:next,returnToDetails:false}));};
