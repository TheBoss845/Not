...
export default function MediaCard({item,rank,progressValue=0,onInfo,onPlay}){
  const{myList,toggleMyList,settings,profile,displayMatchFor}=useNotflix();
  const[expanded,setExpanded]=useState(false);const timer=useRef(null);const inList=myList.includes(item.id);const match=displayMatchFor(item);
  ...
  const showPreview=expanded&&settings.autoplayPreviews&&profile?.autoplay!==false;
  return <article className={`media-card-wrap ${rank?'media-card-wrap--ranked':''}`} data-title-id={item.id}>{rank&&<div className="rank-number">{rank}</div>}<div className={`media-card ${expanded?'media-card--expanded':''}`} onMouseEnter={startExpand} onMouseLeave={stopExpand} onFocusCapture={startExpand} onBlurCapture={stopExpand}>
    <FocusButton className="media-card__art" onClick={()=>onInfo(item)} aria-label={`${item.title}, more info`}>
      ...
      {showPreview&&item.media?.trailer&&<video className="media-card__preview-video" src={item.media.trailer} autoPlay muted loop playsInline/>}
      {showPreview&&!item.media?.trailer&&<div className="media-card__generated-preview"><MotionReel item={item} trailer preview/></div>}
