...
export default function Hero({item,onPlay,onInfo}){
  ...
  if(!item)return null;
  const customTrailer=Boolean(item.media?.trailer);
  return <section className={`hero ${previewing?'hero--previewing':''}`}><div className="hero__visual"><Artwork item={item} variant="hero" previewing={previewing}/>{previewing&&customTrailer&&<video ref={videoRef} className="hero__video" src={item.media.trailer} autoPlay muted={muted} loop playsInline/>}{previewing&&!customTrailer&&<div className="hero__generated-preview"><MotionReel item={item} trailer preview/></div>}</div><div className="hero__vignette"/>
    <div className="hero__content"><div className="eyebrow">{item.kicker}</div><h1>{item.title}</h1><p className="hero__tagline">{item.tagline}</p><p className="hero__description">{item.description}</p><div className="hero__meta"><strong>{item.match}% Match</strong><span>{item.year}</span><span className="rating-pill">{item.rating}</span><span>{item.runtime}</span><span className="hd-pill">{item.media?.source?'FULL FILM':'MOTION CUT'}</span></div><div className="hero__actions"><FocusButton className="hero-button hero-button--play" onClick={()=>onPlay(item)}><Play fill="currentColor" size={24}/> Play</FocusButton><FocusButton className="hero-button hero-button--info" onClick={()=>onInfo(item)}><Info size={24}/> More Info</FocusButton></div></div>
