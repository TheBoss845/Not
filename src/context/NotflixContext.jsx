...
export function NotflixProvider({children}){
  ...
  const[prank,setPrank]=useState(null);const[toast,setToast]=useState(null);const[remoteState,setRemoteState]=useState({connected:false,lastCommand:null});const[viewerTelemetry,setViewerTelemetry]=useState(null);
  const[injected,setInjected]=useState(()=>loadJSON('injected',[]));
  const[customTitles,setCustomTitles]=useState(()=>loadJSON('customTitles',[]));
  const[contentOverrides,setContentOverrides]=useState(()=>loadJSON('contentOverrides',{}));
  ...
  const removeCustomTitle=useCallback((id)=>{setCustomTitles((prev)=>prev.filter((x)=>x.id!==id));setInjected((prev)=>prev.filter((x)=>x!==id));setContentOverrides((prev)=>{const next={...prev};delete next[id];return next;});setProfileData((prev)=>Object.fromEntries(Object.entries(prev).map(([key,data])=>[key,{...data,myList:(data.myList??[]).filter((x)=>x!==id)}])));},[]);
  const updateTitleContent=useCallback((id,patch)=>{setContentOverrides((prev)=>{const current=prev[id]??{};const mediaPatch=patch.media?{media:{...(current.media||{}),...patch.media}}:{};return{...prev,[id]:{...current,...patch,...mediaPatch}};});recordEvent('Updated title media',{titleId:id});},[recordEvent]);
  const clearTitleContent=useCallback((id)=>setContentOverrides((prev)=>{const next={...prev};delete next[id];return next;}),[]);
  const saveRuleOverride=useCallback((id,rules)=>setRuleOverrides((prev)=>({...prev,[id]:rules})),[]);const clearRuleOverride=useCallback((id)=>setRuleOverrides((prev)=>{const next={...prev};delete next[id];return next;}),[]);
